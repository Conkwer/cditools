...
def main():
    ...
    romname = "mygame"
    data_mode = (lba != 11702)  # audio/data at 11702, data/data otherwise
    ...
    try:
        lba = size_sanity(lba, SECTOR_SIZE, OVR1)
        data_verification(binary)
        image_building(romname, TOT_SECTORS, LOUT, SECTOR_SIZE, MARGIN_SECTORS, OVR1, data_mode, lba)
...
def image_building(romname, TOT_SECTORS, LOUT, SECTOR_SIZE, MARGIN_SECTORS, OVR1, data_mode, lba):
    ...
    # Building CDI (using cdibuilder merge mode — 100% open source)
    print("Building CDI...")
    cdi_type = "data" if data_mode else "audio"
    run_command(["./system/cdibuilder", "-M", "session1.bin", "session2.bin",
                  "-o", "output.cdi", "-l", str(lba), "-t", cdi_type])
